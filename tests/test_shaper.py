...
class TestShaperEndpoint:
    ...
    def test_standard_upload(self, client, mock_cli):
        ...
        resp = client.post('/shaper', files=[
            ('files', ('raw_data_x.csv', csv, 'text/csv')),
            ('files', ('raw_data_y.csv', csv, 'text/csv')),
        ])
        assert resp.status_code == 200
        data = resp.json()
        assert data['type'] == 'shaper'
        assert data['printer'] == 'default'
        assert '/results/default/' in data['url']
        assert data['url'].endswith('_shaper.png')

    def test_busybox_separate_fields(self, client, mock_cli):
        csv = make_csv_bytes()
        resp = client.post('/shaper', files=[
            ('file_x', ('raw_data_x.csv', csv, 'text/csv')),
            ('file_y', ('raw_data_y.csv', csv, 'text/csv')),
        ])
        assert resp.status_code == 200
        assert resp.json()['type'] == 'shaper'
    ...
    def test_custom_printer(self, client, mock_cli):
        ...
        assert '/results/k1v3/' in data['url']
    ...
    def test_custom_timestamp(self, client, mock_cli):
        ...
        assert '20260325_120000_shaper.png' in resp.json()['url']

    def test_gzip_upload(self, client, mock_cli):
        gz = make_gzip_csv()
        resp = client.post('/shaper', files=[
            ('files', ('raw_data_x.csv.gz', gz, 'application/gzip')),
            ('files', ('raw_data_y.csv.gz', gz, 'application/gzip')),
        ])
        assert resp.status_code == 200
    ...
    def test_extra_params_passed_to_cli(self, client, mock_cli):
        ...
        assert resp.status_code == 200
        # Verify CLI received the params
        cmd = mock_cli[0][0]
